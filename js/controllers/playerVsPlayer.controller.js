...
app.controller('playerVsPlayerController', function ($scope, $http,$rootScope){
  ...
  $('.selectedTwo').css("display","none");
   $('#map').css("display","none");
    ...
    $scope.showStateData = function(stateName){
        ...
        console.log(stateName + "Here");
        //console.log($scope.category);
        empty($rootScope.topUsers);
        for(var i=0;i< $scope.userData.length;i++){
          //console.log($scope.userData[i].business_name);
                if( $scope.userData[i].business_category == $scope.category){
            //        console.log($scope.userData[i].full_state);
        ...
        //$scope.showStateData('Arizona');
        console.log($scope.topUsers);
        var div1 = $("#topUsersDonut");  
              div1.empty();
              for(var i = 0;i<$rootScope.topUsers.length;i++){
                  var elem1 = $('<button/>', {
                    text: $rootScope.topUsers[i].UserName, //set text 1 to 10
                    id: $rootScope.topUsers[i].UserId,
                    class: 'nameBtnDonut',
                    click: function () { $scope.selectUser(this.id) },
                    disabled: false
                  });
                  div1.append(elem1);
              }
              var elem1 = $('<button/>', {
                    text: "Clear Selection", //set text 1 to 10
                    class: 'categoryBtn specBtn',
                    id: 'clear',
                    click: function () { $scope.clearSelection() }
                  });
              div1.append(elem1);
        ...
        for(var i = 0;i<$rootScope.topUsers.length;i++){
          //var elem1 = $("<button ng-click ='selectUser()' ></button>").text($rootScope.topUsers[i].UserName);
          //var elem2 = $("<div></div>").text($rootScope.topUsers[i].RatingOfUser);
          

          //div.append(elem2);
          //var row = "<div class='row' id='"+$rootScope.topUsers[i].UserId+"'></div>";
          //div.append(row);
          //var col = "<div class='col-md-4'></div>";

          var elem1 = $('<button/>', {
            text: $rootScope.topUsers[i].UserName,
            value: $rootScope.topUsers[i].UserName, //set text 1 to 10
             id: $rootScope.topUsers[i].UserId,
             class: 'nameBtn',
             click: function () { $scope.selectUser1(this.id, this.value); }
          });

          //div.append(elem1);


          var html = $("<div class='row nameRow'>").append($("<div class='col-md-8 paddingZero'>").append(elem1))
    ...
    $scope.selectUser = function(id){
      ...
      if($scope.list.length==2){
        ...
        if($scope.list.length == 2){
            $scope.disable = true;
            $('#topUsersDonut').children().prop('disabled','true');
  ...
  $('.selectedTwo').css("display","none");
      //$scope.update = true;
      $scope.disable = false;
      $('#topUsersDonut').children().removeClass("nameBtnDonutSpec1");
       $('#topUsersDonut').children().removeClass("nameBtnDonutSpec2");
       //console.log($('#topUsersDonut').children());
      $('#topUsersDonut').children().prop('disabled',false);
      empty($scope.list);
    }
    ...
    $scope.selectUser1 = function(id, name){
      ...
      for(var i=0;i<$rootScope.userData.length;i++){
        if($rootScope.userData[i].user_id == id){
          console.log($rootScope.userData[i].business_category);
      ...
      console.log($scope.restaurantDataStateWiseList);
        ...
        for(var i = 0;i<$scope.restaurantDataStateWiseList.length;i++){
          ...
          var elem1 = $('<button/>', {
            text: $scope.restaurantDataStateWiseList[i].RestaurantName, //set text 1 to 10
             class: 'nameBtn1',
             click: function () { initMap(lat, long); }
          });

          //div.append(elem1);


          var html = $("<div class='row nameRow1'>").append($("<div class='col-md-8 paddingZero'>").append(elem1))
          .append($("<div class='col-md-4 paddingZero rank1 '>").append(''+$scope.restaurantDataStateWiseList[i].RestaurantRating+''));
          div.append(html);
          //console.log(html);
        }
    }
    ...
    $scope.displayDonutCharts = function(){
      console.log("displaying1");
        ...
        MonthCount.create($scope.MonthCountData);
         // console.log( $scope.MonthCountData);
         $(".user1").text($scope.MonthCountData[0].data[0].cat); 
         $(".user2").text($scope.MonthCountData[0].data[1].cat); 
        //console.log("displaying3");
        //console.log("Here2");
        //console.log($scope.UserReviewCountData);
        //console.log($scope.UsefullReviewCountData);
       // }
       /*
        else{
        $scope.UserReviewCountData = setData('Total Reviews');
        $('#UserReviewCount').empty();
        UserReviewCount.update($scope.UserReviewCountData);  

        $scope.UsefullReviewCountData = setData('Usefull Reviews');
        UsefullReviewCount.update($scope.UsefullReviewCountData);  

        $scope.FanCountData = setData('Fans');
        FanCount.update($scope.FanCountData);  

        $scope.EliteCountData = setData('Elite Count');
        EliteCount.update($scope.EliteCountData);  

        $scope.ComplimentCountData = setData('Compliments');
        ComplimentCount.update($scope.ComplimentCountData); 

        $scope.MonthCountData = setData('Months on Yelp');
        MonthCount.update($scope.MonthCountData);  
        }
        */
    }
        ...
        for(var i =0; i<$scope.list.length;i++){
           for(var j = 0; j<$rootScope.topUsers.length;j++){
                if($scope.list[i] == $rootScope.topUsers[j].UserId){
                  //console.log($scope.topUsers[j].UserReviewCountData );
                  ...
                  //console.log($rootScope.topUsers[j]);  
                  $scope.data.push({
...
//console.log(valueById);
  ...
  this.updateData = function(data){
      ...
      //console.log(d['state_or_territory']);
...
function DonutCharts(id){
        ...
        var chart_m,
            ...
            color = d3.scale.linear() 
                      .range([d3.rgb("#28B463"), d3.rgb('#D68910')]);
        //console.log(charts);
        ...
        var createCenter = function(pie) {
            ...
            /*donuts.append('text')
                    ...
                    .style('font-weight', 'bold')
                    .text(function(d, i) {
                        return "Average of Both Players";
            });*/
            /*
            donuts.append('text')
                    .attr('class', 'center-txt type2')
                    .attr('y', chart_r * 0.05)
                    .attr('text-anchor', 'middle')
                    .style('font-weight', 'bold')
                    .text(function(d, i) {
